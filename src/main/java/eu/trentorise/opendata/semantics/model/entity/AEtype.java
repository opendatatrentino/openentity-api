...
abstract class AEtype {
     ...
     *             if not found.
     */
    public AttrDef attrDefByConceptId(String conceptId) {
        checkNotEmpty(conceptId, "Invalid attribute definition locator!");

        Map<String, AttrDef> attrDefs = getAttrDefs();
        AttrDef selectedAttrDef = null;
        for (AttrDef attrDef : attrDefs.values()) {
            if (attrDef.getConceptId()
                       .equals(conceptId)) {
                if (selectedAttrDef == null) {
                    selectedAttrDef = attrDef;
                } else {
                    throw new OpenEntityNotFoundException(
                            "Found two attribute definitions with the same concept id!! Choosing first one "
                                    + selectedAttrDef.getId() + " with concept id " + selectedAttrDef.getConceptId()
                                    + " and name " + selectedAttrDef.getName() + " The other is " + attrDef
                                    + " in etype " + getId());
                }
            }
        }
     ...
     *         entity, or the default string field if the etype repreents a
     *         name-like structure. Returns the empty string otherwise.
     * @see #nameAttrDef()
     */
    @Value.Default
     ...
     *         entity. If it represents a non-name struct or it is not found,
     *         throws {@link OpenEntityNotFoundException}.
     * @throws OpenEntityNotFoundException
     */
    public AttrDef nameAttrDef() {
        ...
        if (ret == null) {
            throw new OpenEntityNotFoundException(
                    "Couldn't find name attr def with URL " + getNameAttrDefId() + " in etype " + getId());
        }
        return ret;
    }
    ...
    public AttrDef descrAttrDef() {
        ...
        if (ret == null) {
            throw new OpenEntityNotFoundException(
                    "Couldn't find description attr def with URL " + getDescrAttrDefId() + " in etype " + getId());
        }
        return ret;
    }
    ...
    public AttrDef attrDefById(String URL) {
        ...
        if (ret == null) {
            throw new OpenEntityNotFoundException(
                    "Couldn't find attribute definition with URL " + URL + " in etype with URL " + getId());
        } else {
            return ret;
        }
    }
    ...
    public static Etype of(String URL, String engName, String itName, String conceptURL, String nameAttrDefURL,
            String descrAttrDefURL, List<AttrDef> attrs) {
