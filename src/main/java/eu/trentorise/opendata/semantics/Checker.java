...
public final class Checker {
     ...
     */
    public static void checkEtype(Etype etype) {
        if (etype == null) {
         ...
         * etype.getURL()); }
         */
        for (AttrDef attrDef : etype.getAttrDefs()
                                    .values()) {
            try {
     ...
     */
    public static void checkAttrDef(AttrDef attrDef) {
        if (attrDef == null) {
        ...
        if ((attrDef.getType()
                    .getDatatype()
                    .equals(DataTypes.STRUCTURE)
                || attrDef.getType()
                          .getDatatype()
                          .equals(DataTypes.ENTITY))) {

            checkNotDirtyUrl(attrDef.getType()
                                    .etypeId(),
                    "Attr def " + attrDef.getId() + " is of datatype " + attrDef.getType()
                                                                                .getDatatype()
                            + ", but is has invalid range Etype URL()");
    ...
    public void checkEkbQuick(IEkb ekb) {
        ...
        if (ekb.getDefaultLocales() == null) {
            throw new IllegalArgumentException("Found null locales list in ekb " + ekb.getClass()
                                                                                      .getCanonicalName());
        }

        if (ekb.getDefaultLocales()
               .get(0) == null) {
            throw new IllegalArgumentException("Found null first locale in ekb " + ekb.getClass()
                                                                                      .getCanonicalName());
        }
    }
    ...
    public void checkIDResult(IdResult idResult) {
        ...
        for (Entity entity : idResult.getEntities()) {
            try {
                if (ekb.getEntityService()
                       .isTemporaryURL(entity.getId())) {
        ...
        if (AssignmentResult.REUSE.equals(idResult.getAssignmentResult())) {
            ...
            if (idResult.getEntities()
                        .isEmpty()) {
                throw new IllegalArgumentException("Found empty entities in idResult with "
                        + idResult.getAssignmentResult() + ". idResult is " + idResult);
            }
        }
    ...
    public void checkEntity(Entity entity, boolean synthetic) {
        ...
        Etype etype = ekb.getEtypeService()
                         .readEtype(entity.getEtypeId());
    ...
    public void checkStruct(AStruct struct, boolean synthetic) {
        ...
        checkArgument(etype.getId()
                           .equals(struct.getEtypeId()),
                "Provided etype " + etype.getId() + " is not the one referenced by the struct, which is "
                        + struct.getEtypeId());
        ...
        for (String attrDefId : struct.getAttrs()
                                      .keySet()) {
            try {
    ...
    public void checkAttr(Attr attr, boolean synthetic, AttrDef attrDef) {
        ...
        checkArgument(attr.getAttrDefId()
                          .equals(attrDef.getId()),
    ...
    public static void checkObj(Object obj, AttrType type, boolean synthetic) {
        ...
        if (DataTypes.getDataTypes()
                     .get(type.getDatatype()) == null) {
            throw new IllegalArgumentException("Found unsupported datatype " + type.getDatatype() + " in value " + obj
                    + ". Its class is " + obj.getClass()
                                             .getName());
        }

        if (!(DataTypes.getDataTypes()
                       .get(type.getDatatype())
                       .isInstance(obj))) {
            ...
            if ((type.getDatatype()
                ...
                throw new IllegalArgumentException("Found value not corresponding to its datatype " + type.getDatatype()
                        + ". Value is " + obj + ". Its class is " + obj.getClass()
                                                                       .getName());
            }
        }
    ...
    public void checkValue(Val value, AttrDef attrDef, boolean synthetic) {
        ...
        String datatype = attrDef.getType()
                                 .getDatatype();
